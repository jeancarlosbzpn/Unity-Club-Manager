...
async function findBackups() {
  const col = await getDocs(collection(db, 'club_vencedores_units'));
  console.log("club_vencedores_units size:", col.size);
  const col2 = await getDocs(collection(db, 'units'));
  console.log("units size:", col2.size);
}
findBackups();
